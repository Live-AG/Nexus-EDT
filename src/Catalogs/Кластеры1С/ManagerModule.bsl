...
Процедура СоздатьБазыПоДаннымКластера(Кластер) Экспорт
	...
	ЭлементБлокировки = БлокировкаКластера.Добавить("РегистрСведений.ТекущиеЗаданияОбъектов");
	ЭлементБлокировки.УстановитьЗначение("ИсточникЗадания", Кластер);
...
Процедура ЗарегестрироватьИнформационнуюБазу(Кластер,
	...
	Исключение
		...
		Исключение
			...
			ПроверкаСостоянияОбъектов.ЗаписатьРезультатПроверки(ИнформационнаяБазаСсылка, Правило);
		КонецПопытки;
	КонецПопытки;
	...
	ИнформационнаяБаза.СерверБД		= Справочники.СерверыБД.ПолучитьСерверПоИмени(АдминистрированиеБазыДанных, Истина);

...
Процедура СценарииГотово(Команда)
	...
	Если Не Отказ Тогда
		СоздатьИнформационнуюБазуНаСервере();
		Закрыть();
	КонецЕсли;
...
&НаСервере
Процедура СоздатьИнформационнуюБазуНаСервере()

	//TODO: требуется рефакторинг для соблюдения стандартов
	...
	Попытка
		Записать();
		ИдентификаторИД = Справочники.ИнформационныеБазы.СоздатьИнформационнуюБазуВКластере(Объект.Ссылка, Отказ);
		Если Не Отказ Тогда
			Объект.ИдентификаторБазы = ИдентификаторИД;
			Записать();
		Иначе
			ОтменитьТранзакцию();
			Возврат;	

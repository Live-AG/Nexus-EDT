...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УстановитьЗначенияЭлементовФормы();
	//Отладка
	УстановитьОтображениеНезаполненныхЭлементов();	
	
	//#TODO Получение Свойств скриптами
	ЖурналРегистрации = "Не доступен";
	ОбъемБазы = "Не доступен";
...
КонецПроцедуры
 ...
 &НаСервере
Процедура УстановитьЗначенияЭлементовФормы()

	КоличествоВебСерверов = Объект.ВебСерверы.Количество();

	Если КоличествоВебСерверов = 1 Тогда
		ВебСерверПредставление = Строка(Объект.ВебСерверы[0].ВебСервер);
	ИначеЕсли КоличествоВебСерверов > 1 Тогда
		ВебСерверПредставление = Строка(Объект.ВебСерверы[0].ВебСервер) + "...(" + 
											Формат(КоличествоВебСерверов, "ЧГ=0;");
	КонецЕсли;
...
Процедура КонфигурацияНажатие(Элемент, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.Конфигурация) Тогда
		...
		ОткрытьФорму("Справочник.Конфигурации.ФормаОбъекта", 
					Новый Структура("Ключ", Объект.Конфигурация), 
					Элемент, ЭтаФорма, 
					, , , 
	...
	Иначе                                                                                                                 
		...
		СтруктураПараметров.Вставить("ИмяРеквизита", "Конфигурация");
		...
		ОткрытьФорму("Справочник.Конфигурации.ФормаВыбора", 
...
Процедура ОбработатьДанныеАвторизации(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Результат) Тогда
		...
		ВыполнитьЗаполнениеСвойствБазы(Истина);
	КонецЕсли;
...
Процедура УстановитьОтображениеНезаполненныхЭлементов()
	...
	МассивПроверяемыхРеквизитов.Добавить("Конфигурация");
	...
	МассивПроверяемыхРеквизитов.Добавить("ВебСерверы");
	...
	Для Каждого ПроверяемыйРеквизит Из МассивПроверяемыхРеквизитов Цикл
		...
		ЗначениеРеквизитаЗаполнено	= ЗначениеЗаполнено(Объект[ПроверяемыйРеквизит]);
		ЭлементПустогоРеквизита		= Элементы["Группа" + ПроверяемыйРеквизит + "Пустая"]; 
		ЭлементРеквизита			= Элементы[ПроверяемыйРеквизит];
...
&НаКлиенте
Процедура ВыполнитьЗаполнениеСвойствБазы(ОбновитьДанныеСУБД = Ложь)
	
	ДлительнаяОперация = НачатьПолучениеСвойствБазы(ОбновитьДанныеСУБД);
...
&НаСервере
Функция НачатьПолучениеСвойствБазы(ОбновитьДанныеСУБД)
...
Процедура ДекорацияНазначениеНеЗаполненоНажатие(Элемент)
	...
	ВыбратьЗначениеРеквизита(Элемент, "Назначение", "Справочник.НазначенияИБ.ФормаВыбора");
...
Процедура ДекорацияВерсияКонфигурацииНеЗаполненоНажатие(Элемент)
	...
	ВыбратьЗначениеРеквизита(Элемент, "Конфигурация", "Справочник.Конфигурации.ФормаВыбора");
...
Процедура ДекорацияВебСерверНеЗаполненоНажатие(Элемент)
	...
	ВыбратьЗначениеРеквизита(Элемент, "ВебСерверПредставление", "Справочник.ВебСерверы.ФормаВыбора");
...
Процедура ВыбратьЗначениеРеквизита(Элемент, ИмяРеквизита, ИмяФормы)
	...
	// TODO: Сделать возможность выбора из списка
...
Процедура уд_ОтладкаНаСервере()
	...
	Запрос.Текст = "ВЫБРАТЬ
						...
						|ГДЕ
						|	Конфигурации.ВерсияКонфигурации = &ВерсияКонфигурации
						|	И Конфигурации.ИмяКонфигурации = &ИмяКонфигурации";
	
	Запрос.УстановитьПараметр("ВерсияКонфигурации", СведенияОКонфигурации.ВерсияКонфигурации);
	Запрос.УстановитьПараметр("ИмяКонфигурации", СведенияОКонфигурации.ИмяКонфигурации);
	Результат = Запрос.Выполнить();
	Выборка = Результат.Выбрать();
	
	Если Выборка.Следующий() Тогда
		Объект.Конфигурация = Выборка.Ссылка;
	КонецЕсли;
...
Процедура уд_Отладка(Команда) 
	...
	Значение = Объект.Конфигурация;
	ИмяРеквизита = "Конфигурация";

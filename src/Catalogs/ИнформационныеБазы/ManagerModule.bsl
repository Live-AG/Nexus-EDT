...
Функция ПолучитьСведенияОКонфигурации(ИнформационнаяБаза) Экспорт
	...
	СтрокаСоединения = РаботаСоСкриптамиКлиентСервер.ПолучитьСтрокуСоединенияСБазой(ИнформационнаяБаза); 
...
Функция СоздатьИнформационнуюБазуВКластере(ИнформационнаяБаза, Отказ = Ложь) Экспорт
	
	ИдентификаторБазы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ИнформационнаяБаза, "ИдентификаторБазы");
	Если ЗначениеЗаполнено(ИдентификаторБазы) Тогда
		//TODO: Записать ошибку в лог - "База уже связана с элементом кластера - создание не возможно."
		Отказ = Истина;
	Иначе
		ПараметрыИБ = АдминистрированиеЭлементовКластера.ПолучитьПараметрыИнформационнойБазы(ИнформационнаяБаза);
		ИдентификаторБазы = АдминистрированиеЭлементовКластера.СоздатьИнформационнуюБазу(ПараметрыИБ, Отказ);	
	КонецЕсли;

...
Процедура ПередЗаписью(Отказ)
	...
	Если ЭтоНовый() Тогда
		...
		ПараметрыЗапускаПоУмолчанию.Вставить("ВариантАутентификации",				0);
		ПараметрыЗапускаПоУмолчанию.Вставить("Версия",								"8.3");
		ПараметрыЗапускаПоУмолчанию.Вставить("ДополнительныеПараметрыЗапуска",	"");
		ПараметрыЗапускаПоУмолчанию.Вставить("ОсновнойРежимЗапуска",				"Auto");
		ПараметрыЗапускаПоУмолчанию.Вставить("Разрядность", 						"x86_64_prt");
		
		ПараметрыЗапуска = Новый ХранилищеЗначения(ПараметрыЗапускаПоУмолчанию);
	КонецЕсли;

...
Функция ВыполнитьПроверкуОбъекта(Объект, Правило) Экспорт
	
	Если Правило = ЗаполнениеДанныхСУБД Тогда
		ПроверкаПройдена = Проверить_ЗаполнениеДанныхСУБД(Объект);		
	Иначе
		// TODO	
	КонецЕсли;
	
	Если ПроверкаПройдена Тогда
		ПроверкаСостоянияОбъектов.ЗаписатьРезультатПроверки(Объект, Правило, Истина);
	Иначе
		ПроверкаСостоянияОбъектов.ЗаписатьРезультатПроверки(Объект, Правило);
	КонецЕсли;

...
// * ПользовательЦентральногоСервера - Строка -
...
Функция ПолучитьСтруктуруПараметруКластера() Экспорт
	...
	СтруктураПараметров.Вставить("ПользовательЦентральногоСервера", "");
...
Функция ПолучитьАгентаАдминистрированияКластера(ПараметрыПодключения, Отказ = Ложь) Экспорт
	...
	Попытка
		АгентАдминистрирования.ВыполнитьАутентификацию(ПараметрыПодключения.ПользовательЦентральногоСервера,
...
// 
// Параметры:
//  Кластер - СправочникСсылка.Кластеры1С, Структура - Свойства кластера
//  Отказ - Булево - Отказ
// 
// Возвращаемое значение:
...
// 
// Параметры:
//  ПараметрыИБ - Структура - Параметры базы
//  Отказ - Булево
//  
// Возвращаемое значение:
//	 УникальныйИдентификатор - Идентификатор информационной базы
Функция СоздатьИнформационнуюБазу(ПараметрыИБ, Отказ) Экспорт

	ЭлементАдминистрированияКластера = ПолучитьАдиминистрированиеКластера(ПараметрыИБ.Кластер, Отказ);
	...
	//•ЗапретитьЛокальноеРаспознаваниеРечи
	...
	ЗаполнитьЗначенияСвойств(ЭлементАдминистрированияИБ, ПараметрыИБ);

	СоответствиеПолей = Новый Соответствие;
	СоответствиеПолей.Вставить("Имя",								"ИмяИБ");
	СоответствиеПолей.Вставить("ИмяБазыДанных",					"БазаДанных");
	СоответствиеПолей.Вставить("ПользовательБазыДанных",		"ИмяАдминистратораСУБД");
	СоответствиеПолей.Вставить("ПарольПользователяБазыДанных",	"ПарольАдминистратораСУБД");
	СоответствиеПолей.Вставить("СерверБазДанных",					"СерверБД");

	ЗаполнитьЗначенияСвойствПоСоответствию(ЭлементАдминистрированияИБ, ПараметрыИБ, СоответствиеПолей);
	
	ЭлементАдминистрированияИБ.СУБД = XMLстрока(ПараметрыИБ.ТипСУБД);
	
	//TODO: Вынести значения по умолчанию в доп.настройки, в форме создания базы.
	ЭлементАдминистрированияИБ.СоздатьБазуДанных = Истина;
	ЭлементАдминистрированияИБ.СмещениеДат = 0;
	ЭлементАдминистрированияИБ.КодЯзыка = "ru_RU";
	ЭлементАдминистрированияИБ.СоздатьБазуДанных = Истина;
	ЭлементАдминистрированияИБ.БлокировкаРегламентныхЗаданий = Истина;
		
	ЭлементАдминистрированияИБ.Записать();
	
	Возврат ЭлементАдминистрированияИБ.ИдентификаторИнформационнойБазы; 	
...
Процедура УдалитьИнформационнуюБазу(ИнформационнаяБаза, 
											РежимУдаленияБазы = Неопределено, 
											УдалитьКаталогКЭШа = Ложь) Экспорт
	...
	Попытка
		...
		ЭлементБлокировки.УстановитьЗначение("Ссылка", ИнформационнаяБаза);
		...
		ИнформационнаяБазаОбъект.УстановитьПометкуУдаления(Истина);
		
		Если ЭлемнетАдминистрированияИБ <> Неопределено Тогда
			ЭлемнетАдминистрированияИБ.Удалить(РежимУдаленияБазы);
		КонецЕсли;
		
		Если УдалитьКаталогКЭШа И ЭлемнетАдминистрированияИБ <> Неопределено Тогда
			ИдентификаторБазы = ЭлемнетАдминистрированияИБ.ИдентификаторИнформационнойБазы;
			УдалитьКаталогКЕШаНаСервере(ИдентификаторБазы);
		КонецЕсли;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Не удалось заблокировать элемент'",
			ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка,,,
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(
			ИнформацияОбОшибке()));
		
		ВызватьИсключение "Не удалось заблокировать элемент";
		
	КонецПопытки;
	
	//TODO:
	//Программно получить каталог кеша баз в кластере по идентификатору и удалить	

КонецПроцедуры

Процедура УдалитьКаталогКЕШаНаСервере(ИдентификаторБазы)
	
	//TODO: Реализация
	
КонецПроцедуры

// Установить счетчик потребления ресурсов.
// 
// Параметры:
//  Кластер - СправочникСсылка.Кластеры1С - Кластер
//  СчетчикВключен - Булево - Счетчик включен
Процедура УстановитьСчетчикПотребленияРесурсов(Кластер, СчетчикВключен) Экспорт

	Отказ = Ложь;
	Администрирование_Кластера = ПолучитьАдиминистрированиеКластера(Кластер, Отказ);
	Если Не Отказ Тогда
		//TODO Включение счетчиков кластера только при лицензии КОРП
		//СчетчикПотребленияРесурсов = Администрирование_Кластера.
	КонецЕсли;
...
Функция ПолучитьАдминистрированиеИнформационнойБазы(ИнформационнаяБаза, Отказ = Ложь) Экспорт
	...
	//TODO: Параметры Аутентификации перенести в Безопасное хранилище  
	СписокРеквизитов = "ИдентификаторБазы, Владелец, ИмяПользователяИБ, ПарольПользователяИБ";
	РеквизитыИнформационнойБазы = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ИнформационнаяБаза, СписокРеквизитов);
	
	ИдентификаторИнформационнойБазы = РеквизитыИнформационнойБазы.ИдентификаторБазы;
	МассивИнформационныхБаз = ПолучитьБазыКластера(РеквизитыИнформационнойБазы.Владелец);
	Для Каждого АдминистрированиеИнформационнойБазы Из МассивИнформационныхБаз Цикл
		...
		Если АдминистрированиеИнформационнойБазы.ИдентификаторИнформационнойБазы = ИдентификаторИнформационнойБазы Тогда
			Попытка
				АдминистрированиеИнформационнойБазы.ВыполнитьАутентификацию(РеквизитыИнформационнойБазы.ИмяПользователяИБ,
					РеквизитыИнформационнойБазы.ПарольПользователяИБ);
			Исключение
				Отказ = Истина;
			КонецПопытки;
...
// * ПользовательЦентральногоСервера 
...
Функция ПолучитьСтруктуруПараметровИнформационнойБазы()
	...
	СтруктураПараметров.Вставить("Кластер");
	СтруктураПараметров.Вставить("АдресСервераАдминистрирования");
	...
	СтруктураПараметров.Вставить("ТипСУБД");
...
Функция ПолучитьПараметрыИнформационнойБазы(ИнформационнаяБаза) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
					|	ИнформационныеБазы.Владелец КАК Кластер,
					...
					|	ИнформационныеБазы.Владелец.ПользовательЦентральногоСервера КАК ПользовательЦентральногоСервера,
					...
					|		Порт КАК РабочиеСерверыПорт) КАК РабочиеСерверы,
					...
					|	ИнформационныеБазы.СерверБД.ТипСУБД КАК ТипСУБД,
					|	ИнформационныеБазы.Ссылка КАК ИнформационнаяБаза,
					|	ИнформационныеБазы.Описание КАК Описание
					|ИЗ
	...
	Если Выборка.Следующий() Тогда
		...
		РабочиеСерверыВыборка = Выборка.РабочиеСерверы.Выбрать();
		Пока РабочиеСерверыВыборка.Следующий() Цикл 
			СоответствиеРабочихCерверов.Вставить(РабочиеСерверыВыборка.РабочиеСерверыИмя, 
															РабочиеСерверыВыборка.РабочиеСерверыПорт);
		КонецЦикла;
...
Функция ПолучитьСвойстваИнформационнойБазы(ИнформационнаяБаза, ОбновитьДанныеСУБД = Ложь) Экспорт
	...
	СерверБазДанных = Справочники.СерверыБД.ПолучитьСерверПоИмени(Адимнистрирование_ИнформационнаяБаза, 
																				ОбновитьДанныеСУБД);
	СтркутураСвойствБазы.Вставить("СерверБазДанных", СерверБазДанных);
...
// Заполнить значения свойств по соответствию.
// 
// Параметры:
//  Приемник - Произвольный - Коллекция с возможностью получения элементов по ключу
//  Источник - Произвольный - Коллекция с возможностью получения элементов по ключу
//  Соответствие - Соответствие из КлючИЗначение:
//						* Ключ     - Строка - свойство приемника.
//						* Значение - Строка - свойство источника.  
Процедура ЗаполнитьЗначенияСвойствПоСоответствию(Приемник, Источник, Соответствие) Экспорт
	
	Для Каждого ЭлементСоответствия Из Соответствие Цикл
		Приемник[ЭлементСоответствия.Ключ] = Источник[ЭлементСоответствия.Значение]; 	
	КонецЦикла;

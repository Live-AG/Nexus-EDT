...
Процедура СобратьСтатистикуИспользованияИнформационныхБаз() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		ТаблицаАктивныхСеансов = АдминистрированиеЭлементовКластера.ПолучитьТаблицуАктивныхСеансовКластера(Выборка.Кластер);
		Если ТаблицаСеансов = Неопределено Тогда
			ТаблицаСеансов = ТаблицаАктивныхСеансов;
		Иначе
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаСеансов, ТаблицаАктивныхСеансов);
		КонецЕсли;		
	КонецЦикла;
	
	Если ТаблицаСеансов <> Неопределено И ТаблицаСеансов.Количество() > 0 Тогда

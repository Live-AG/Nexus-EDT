...
&НаКлиенте
Процедура Тест_ОбработкаДействийКластер(Команда)
	...
	Отладка_ОбработкаДействийКластер(Элементы.ТаблицаЭлементовКластера.ТекущаяСтрока);
	
	
КонецПроцедуры 

&НаСервереБезКонтекста
Процедура Отладка_ОбработкаДействийКластер(Кластер)

	Справочники.Кластеры1С.СоздатьБазыПоДаннымКластера(Кластер);
...
Процедура ТаблицаИнформационныхБазПриАктивизацииСтроки(Элемент)
	...
	ЗаполнитьОтображениеМенюИзбранное(Элементы.ТаблицаИнформационныхБаз.ТекущаяСтрока);	
...
Процедура ЗаполнитьОтображениеМенюИзбранное(ИнформационнаяБаза)
	...
	Если ЗначениеЗаполнено(ИнформационнаяБаза) 
		И ТипЗнч(ИнформационнаяБаза) = Тип("СправочникСсылка.ИнформационныеБазы") Тогда
		
		ЭтоСтрокаБазы = Истина;
		
		СтруктураПоиска = Новый Структура("ИнформационнаяБаза", ИнформационнаяБаза);
		МассивСтрокаИзбранного = ТаблицаСоответствияИзбранного.НайтиСтроки(СтруктураПоиска);
	...
	Элементы.ТаблицаИнформационныхБазКонтекстноеМенюПоделитьсяПодключением.Доступность	= ЭтоСтрокаБазы;
	Элементы.ТаблицаИнформационныхБазКонтекстноеМенюГруппаПоделиться.Доступность			= ЭтоСтрокаБазы; 
	Элементы.ТаблицаИнформационныхБазКонтекстноеМенюГруппаИзбранное.Доступность			= ЭтоСтрокаБазы;
	Элементы.ТаблицаИнформационныхБазКонтекстноеМенюГруппаЗапуск.Доступность				= ЭтоСтрокаБазы;

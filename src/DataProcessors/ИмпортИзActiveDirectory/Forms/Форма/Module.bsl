...
Процедура ЗагрузитьПользователейКлиентФрагмент()   
	...
	ТЗРезультаты = ВыполнитьИмпортИзАД(ТаблицаПользователейАД, ПрофильИмпортаИзAD, Истина);
	//
...
Функция ВыполнитьИмпортИзАД(ТаблицаПользователейАД, ПрофильИмпортаИзAD, Интерактивно = Ложь) Экспорт
	...
	Для Каждого Строка Из ТаблицаПользователейАД Цикл
		НайтиСоздатьПользователяИБ(Строка);
	КонецЦикла;
...
Функция НайтиСоздатьПользователяИБ(ДанныеПользователя)
	
	ПользовательИБ = Справочники.ПользователиИБ.НайтиПоРеквизиту("ИмяВДомене", ДанныеПользователя.Логин);
	Если ЗначениеЗаполнено(ПользовательИБ) Тогда
		Возврат ПользовательИБ;
	Иначе
		ПользовательИБ = Справочники.ПользователиИБ.СоздатьЭлемент();
		ПользовательИБ.Активный			= Истина;
		ПользовательИБ.Почта				= СокрЛП(ДанныеПользователя.Почта);
		ПользовательИБ.Телефон				= СокрЛП(ДанныеПользователя.Телефон);
		ПользовательИБ.Должность			= СокрЛП(ДанныеПользователя.Должность);
		ПользовательИБ.Подразделение		= СокрЛП(ДанныеПользователя.Подразделение);
		ПользовательИБ.Организация		= СокрЛП(ДанныеПользователя.Организация);
		ПользовательИБ.ПользовательОС		= "\\" + СокрЛП(ДанныеПользователя.Домен) + "\" + СокрЛП(ДанныеПользователя.Логин);
		ПользовательИБ.ИмяВДомене			= СокрЛП(ДанныеПользователя.Логин);
		ПользовательИБ.Наименование		= ФизическиеЛицаКлиентСервер.ФамилияИнициалы(СокрЛП(ДанныеПользователя.Имя));
		ПользовательИБ.ПолноеИмя			= СокрЛП(ДанныеПользователя.Имя);
